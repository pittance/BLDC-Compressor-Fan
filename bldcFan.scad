...
bladeThick = 3.2;       //thickness of rotor blades
...
module intakeShroud() {
    ...
    intersection() {
        casing();
        cylinder(d=180,h=121.022,$fn=10,convexity=2);
    }
}
module motorShroud() {
    //clipped from casing
...
module casing() {
    rotate_extrude($fn=detail,convexity=5)translate([-199.5,0,0])import (file="fanLayout.dxf",layer="casing",convexity=5);
}
module motor() {
...
module fan() {
    ...
    difference() {
        ...
        for(i=[1:4]) {
            rotate([0,0,i*360/4])translate([9/2,0,43])cylinder(h=80,d=4.5,$fn=detail);
        }
        for(i=[1:4]) {
...
function out(ht,tw,maxHt)= [for(i=[0:nStans-1])[(x(i/nStans)*cos(tw)+yp(i/nStans,ht/nSteps)*sin(tw)),(-x(i/nStans)*sin(tw)+yp(i/nStans,ht/nSteps)*cos(tw)),maxHt*ht/nSteps]];
// was:
// function out(ht,tw,maxHt)= [for(i=[0:nStans-1])[(x(i/nStans)*cos(tw)+yp()*sin(tw)),(-x(i/nStans)*sin(tw)+yp()*cos(tw)),maxHt*ht/nSteps]];

function back(ht,tw,maxHt)= [for(j=[nStans-1:-1:0])[(x(j/nStans)*cos(tw)+yn(j/nStans,ht/nSteps)*sin(tw)),(-x(j/nStans)*sin(tw)+yn(j/nStans,ht/nSteps)*cos(tw)),maxHt*ht/nSteps]];
// was:
// function back(ht,tw,maxHt)= [for(j=[nStans-1:-1:0])[(x(j/nStans)*cos(tw)+yn()*sin(tw)),(-x(j/nStans)*sin(tw)+yn()*cos(tw)),maxHt*ht/nSteps]];

// x(p) parametric function of x (out along blade)
function x(p) = (p*bladeSpan);
// yp() y in the positive direction
function yp(p,q) = (bladeThick/2)+bladeSweep*p*p*(-q+1); //was function yp() = (bladeThick/2);
// yn() y in the negative direction
function yn(p,q) = -(bladeThick/2)+bladeSweep*p*p*(-q+1); //was function yp() = (bladeThick/2);
